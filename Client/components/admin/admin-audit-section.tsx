...
export function AdminAuditSection() {
  ...
  const setFilter = useCallback(
    (patch: Partial<AuditFilters>, resetPage = true) => {
      setFilters((prev) => ({
        ...prev,
        ...patch,
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      const response = await listAdminAuditLogs(token, {
        ...
        collection:
          filters.collection !== "all" ? filters.collection : undefined,
        success:
    ...
    <div className="admin-panel-shell admin-animate-card">
      ...
      <div className="relative z-10 mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="admin-metric-card">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Eventos con error en la vista actual.
          </p>
        </div>
        <div className="admin-metric-card">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {filters.limit} eventos por pagina.
          </p>
        </div>
        <div className="admin-metric-card">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {filters.view === "important"
              ? "Vista enfocada en eventos clave."
              : "Vista completa del registro."}
          </p>
        </div>
      </div>

      <div className="admin-form-card relative z-10 mt-5">
        <div className="flex flex-col gap-4 border-b border-border/60 pb-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Busca por usuario o ruta y acota el historial por metodo, accion,
              coleccion y resultado.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline">{meta.total} registros</Badge>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[1.45fr_repeat(5,minmax(0,1fr))]">
          ...
          <Select
            value={filters.view}
            onValueChange={(value) =>
              setFilter({ view: value as AuditFilters["view"] })
            }
          >
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilter({ method: value as AuditFilters["method"] })
            }
          >
            <SelectTrigger className="admin-input-surface">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilter({ action: value as AuditFilters["action"] })
            }
          >
            <SelectTrigger className="admin-input-surface">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilter({ collection: value as AuditFilters["collection"] })
            }
          >
            <SelectTrigger className="admin-input-surface">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setFilter({ success: value as AuditFilters["success"] })
            }
          >
            <SelectTrigger className="admin-input-surface">
        ...
        <div className="flex flex-col gap-3 border-b border-border/60 px-4 py-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Sigue cambios de contenido, catalogo, respaldos y eventos tecnicos
              del panel.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
