...
const STATUS_ACTION_MESSAGES: Record<Exclude<AdminOrderStatus, "draft">, string> = {
...
function canCancel(status?: AdminOrderStatus) {
  return status === "draft" || status === "pending_review" || status === "confirmed"
}
...
export function AdminOrdersSection() {
  ...
  const [actionLoading, setActionLoading] = useState<AdminOrderStatus | null>(null)
      ...
      try {
        ...
        setErrorMessage(
          error instanceof Error
            ? error.message
            : "No se pudo cargar el modulo de pedidos.",
        )
        setOrders([])
  ...
  const visibleStats = useMemo(
    ...
    [meta.total, summary.completed, summary.confirmed, summary.manualReview, summary.pending_review],
  )
  ...
  const refreshAll = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "No se pudo recargar el modulo de pedidos.",
      )
    } finally {
      setIsLoadingOrders(false)
      setIsLoadingDetail(false)
  ...
  const handleStatusChange = async (nextStatus: AdminOrderStatus) => {
    ...
    try {
      ...
      setActionNote("")
      setSuccessMessage(
        STATUS_ACTION_MESSAGES[nextStatus as Exclude<AdminOrderStatus, "draft">] ??
          "Pedido actualizado.",
      )
      await refreshAll()
    } catch (error) {
    ...
    <section className="admin-panel-shell admin-animate-card">
      <div className="relative z-10 space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.02fr_0.98fr]">
              ...
              <div className="flex flex-col gap-4 border-b border-border/60 pb-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Busca por folio o cliente y enfoca el listado por estado comercial.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline">{meta.total} visibles</Badge>
                  <Badge variant="secondary">{summary.pending_review} pendientes</Badge>
                </div>
              </div>
            ...
            <div className="admin-table-shell">
              <div className="flex flex-col gap-3 border-b border-border/60 px-5 py-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Selecciona un pedido para revisar cliente, inventario y acciones del equipo.
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline">Pagina {meta.page}</Badge>
                  <Badge variant="secondary">{meta.totalPages} vistas</Badge>
                </div>
              </div>
              <Table>
                <TableHeader>
                  <TableRow className="admin-table-head-row">
                    <TableHead className="admin-table-head-cell">Folio</TableHead>
                    <TableHead className="admin-table-head-cell">Cliente</TableHead>
                    <TableHead className="admin-table-head-cell">Total</TableHead>
                    <TableHead className="admin-table-head-cell">Fecha</TableHead>
          ...
          <div className="space-y-5">
            {!selectedOrderId ? (
              <div className="admin-empty-state admin-section-card p-6 text-left">
                <div className="rounded-full bg-primary/10 p-3 text-primary">
                  <ShoppingBag className="h-5 w-5" />
                </div>
                <div>
                  <p className="font-medium text-foreground">Selecciona un pedido</p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Aqui apareceran cliente, aromas, alertas y acciones operativas.
                  </p>
                </div>
              </div>
            ) : isLoadingDetail && !selectedOrder ? (
              <div className="admin-section-card p-6 text-sm text-muted-foreground">
              ...
              <>
                <div className="admin-section-card p-5">
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Badge
                        variant="outline"
                        className={cn("rounded-full", getStatusTone(selectedOrder.status))}
                      >
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-2">
                    <div className="admin-stat-chip">
                      ...
                      {formatPrice(selectedOrder.subtotal, selectedOrder.currency)}
                    </div>
                    <div className="admin-stat-chip">
                      <span className="font-medium">Aromas:</span> {selectedOrder.totalItems}
                    </div>
                    <div className="admin-stat-chip">
                      <span className="font-medium">Canal:</span> {selectedOrder.channel}
                    </div>
                    <div className="admin-stat-chip">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-border/60 bg-background/80 px-4 py-3">
                      ...
                      <p className="mt-2 text-sm text-foreground">
                        {selectedOrder.customerUserEmail ?? "Pedido como invitado"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="admin-section-card p-5">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">Aromas del pedido</p>
                  </div>
                    ...
                    {selectedOrder.items.map((item) => (
                      ...
                      >
                        <div className="flex flex-col gap-4 sm:flex-row">
                          <div className="relative h-28 w-full shrink-0 overflow-hidden rounded-[1.25rem] border border-emerald-100 bg-white sm:w-24">
                            <Image
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                              <div>
                                <div className="flex flex-wrap gap-2">
                                  <Badge
                                    variant="outline"
                                    className={cn("rounded-full", getFulfillmentTone(item.fulfillment))}
                                  >
                ...
                <div className="admin-section-card p-5">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">Historial de estado</p>
                  </div>
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div className="flex flex-wrap items-center gap-2">
                            <Badge
                              variant="outline"
                              className={cn("rounded-full", getStatusTone(note.status))}
                            >
                ...
                <div className="admin-section-card p-5">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-foreground">Acciones del equipo</p>
                  </div>

                  <Textarea
                    value={actionNote}
                    onChange={(event) => setActionNote(event.target.value)}
                    className="admin-input-surface mt-4 min-h-[112px]"
                    maxLength={500}
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <Button
                      ...
                      disabled={!canManage || !canConfirm(selectedOrder.status) || Boolean(actionLoading)}
                    >
                    ...
                    <Button
                      ...
                      disabled={!canManage || !canComplete(selectedOrder.status) || Boolean(actionLoading)}
                    >
                    ...
                    <Button
                      ...
                      disabled={!canManage || !canCancel(selectedOrder.status) || Boolean(actionLoading)}
                    >
                  ...
                  !canCancel(selectedOrder.status) ? (
                    <div className="mt-4 rounded-xl border border-stone-200 bg-stone-100 px-4 py-3 text-sm text-stone-700">
                      Este pedido ya esta en un estado terminal y no admite nuevas acciones.
                    </div>
                  ) : null}

                  <p className="mt-4 text-xs text-muted-foreground">
                    Confirmar mantiene la reserva activa, completar consume la reserva y cancelar
                    libera existencias al inventario.
                  </p>
                </div>
              </>
            )}
