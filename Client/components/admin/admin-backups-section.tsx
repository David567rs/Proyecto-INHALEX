...
function storageLabel(
  storage: AdminBackupSettings["preferredStorage"],
): string {
...
function isPreferredRemoteConfigured(
...
function destinationState(settings: AdminBackupSettings | null): {
  ...
  if (settings.preferredStorage === "local")
    return { title: "Servidor local", ready: true };
  if (settings.preferredStorage === "cloudinary") {
...
export function AdminBackupsSection() {
  ...
  const [importMode, setImportMode] =
    useState<AdminBackupImportMode>("replace");
  const [isLoading, setIsLoading] = useState(true);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [isSavingSettings, setIsSavingSettings] = useState(false);
  const [isCreatingDatabaseBackup, setIsCreatingDatabaseBackup] =
    useState(false);
  const [isCreatingCollectionBackup, setIsCreatingCollectionBackup] =
    useState(false);
  ...
  const projectedProtectedDocuments = useMemo(() => {
    ...
    return sortedCollections.reduce(
      (sum, collection) =>
        sum + (active.has(collection.name) ? collection.count : 0),
      0,
    );
  ...
  const loadAll = async (withLoadingState = true) => {
    ...
    try {
      ...
      setSelectedCollection((current) => {
        const exists = collectionsResponse.items.some(
      ...
      setSelectedBackupId((current) => {
        const exists = backupsResponse.items.some(
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "No se pudo cargar el centro de respaldos",
      );
    } finally {
      setIsLoading(false);
      setIsRefreshing(false);
  ...
  const toggleCollection = (collectionName: string) => {
    ...
    setDraft({
      ...
      selectedCollections: exists
        ...
        : [...draft.selectedCollections, collectionName].sort((a, b) =>
  ...
  const saveSettings = async () => {
    ...
    if (
      draft.backupScope === "selectedCollections" &&
      draft.selectedCollections.length === 0
    ) {
      setErrorMessage(
        "Selecciona al menos una coleccion para la estrategia segmentada.",
      );
      return;
    }
  ...
  const createCollectionBackup = async () => {
    ...
    try {
      const created = await createAdminCollectionBackup(
        selectedCollection,
        token,
      );
      setResultMessage(
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "No se pudo generar el respaldo por coleccion",
      );
    } finally {
  ...
  const runPolicy = async () => {
    ...
    try {
      ...
      setResultMessage(
        `Politica ejecutada. Respaldos creados: ${response.created.length}.`,
      );
      await loadAll(false);
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "No se pudo ejecutar la estrategia",
      );
    } finally {
  ...
  const importBackup = async (backupId: string) => {
    ...
    if (importMode === "replace") {
      const accepted = window.confirm(
        "Se reemplazaran datos actuales con el contenido del respaldo. Deseas continuar?",
      );
      if (!accepted) return;
    }
    ...
    try {
      ...
      setResultMessage(
        `Importacion completada sobre ${imported.collections.length} colecciones.`,
      );
      await loadAll(false);
    } catch (error) {
      setErrorMessage(
    ...
    <div className="relative overflow-hidden rounded-lg border border-border/60 bg-card/95 p-4 lg:p-4 shadow-[0_20px_80px_-50px_rgba(15,23,42,0.45)] backdrop-blur">
      ...
      <div className="relative z-10 space-y-4">
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
              Controla estrategia, ejecucion, restauracion e historial desde un
              solo tablero.
            </p>
          </div>
        </div>

        <div className="admin-toolbar-surface px-3 py-2.5 lg:sticky lg:top-0 lg:z-20">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
              ...
              <Badge variant="secondary">
                {status?.totalReady ?? readyBackups.length} listos
              </Badge>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button
                type="button"
                className="rounded-md px-4 shadow-[0_14px_40px_-18px_rgba(22,163,74,0.8)]"
                onClick={runPolicy}
                disabled={isRunningPolicy || isLoading}
              >
                <Bot
                  className={`h-4 w-4 ${isRunningPolicy ? "animate-pulse" : ""}`}
                />
        ...
        <div className="grid gap-4 xl:grid-cols-[1.12fr_0.88fr]">
          <div className="admin-form-card">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Primero define estrategia y destino, despues ejecuta respaldos
              puntuales cuando haga falta y usa el historial solo para validar,
          ...
          <div className="admin-form-card">
            ...
            <div className="mt-5 grid gap-3">
              ...
              <div className="admin-stat-chip">
                ...
                {draft?.automaticEnabled ? "Activo" : "Pausado"}
              </div>
              <div className="admin-stat-chip">
                <span className="font-medium">Respaldos listos:</span>{" "}
                {status?.totalReady ?? readyBackups.length}
              </div>
              <div className="admin-stat-chip">
                <span className="font-medium">Ultimo exito:</span>{" "}
                {formatDate(settings?.lastSuccessfulRunAt)}
              </div>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 xl:grid-cols-4">
            <Card className="group min-h-[8.5rem] border-primary/15 bg-primary/[0.04] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                ...
                <div className="pt-1">
                  <Badge
                    variant={destinationInfo.ready ? "secondary" : "outline"}
            ...
            <Card className="min-h-[8.5rem] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
              <CardHeader className="gap-1">
            ...
            <Card className="min-h-[8.5rem] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                <p>
                  Listos: {status?.totalReady ?? 0} | Fallidos:{" "}
                  {status?.totalFailed ?? 0} | Purgados:{" "}
                  {status?.totalPurged ?? 0}
                </p>
            ...
            <div className="grid gap-4 xl:grid-cols-[1.3fr_0.95fr]">
              <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>
                    Define objetivos de continuidad, alcance automatico y
                    cobertura operativa.
                  </CardDescription>
                </CardHeader>
                    ...
                    <>
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Minimo 15 y maximo 10080. Define cada cuanto debe
                            ejecutarse la politica automatica.
                          </p>
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-foreground">
                          ...
                          />
                          <p className="text-xs text-muted-foreground">
                            Minimo 5 y maximo 1440. Sirve como meta interna de
                            restauracion.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              setDraft({
                                ...draft,
                                backupScope:
                                  value as BackupSettingsDraft["backupScope"],
                              })
                            }
                          >
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="selectedCollections">
                        ...
                        <div className="space-y-2">
                          ...
                          />
                          <p className="text-xs text-muted-foreground">
                            Despues de este tiempo, los respaldos viejos se
                            purgan segun la politica.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="rounded-lg border border-border/60 bg-secondary/20 p-4">
                        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              El servidor revisa cada minuto si el RPO se vencio
                              y ejecuta la politica automaticamente.
                            </p>
                          </div>
                          <Switch
                            checked={draft.automaticEnabled}
                            onCheckedChange={(value) =>
                              setDraft({ ...draft, automaticEnabled: value })
                            }
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Si eliges modo segmentado, solo estas colecciones
                              formaran parte de la politica.
                            </p>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {sortedCollections.length === 0 && (
                            <span className="text-sm text-muted-foreground">
                              No se detectaron colecciones autorizadas.
                            </span>
                          )}
                          {sortedCollections.map((collection) => {
                            const active = draft.selectedCollections.includes(
                              collection.name,
                            );
                            const dimmed =
                              draft.backupScope !== "selectedCollections";
              ...
              <Card className="overflow-hidden border-primary/10 bg-[linear-gradient(180deg,rgba(22,163,74,0.04),transparent_42%)] transition-all duration-300 hover:shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>
                    Decide donde viven los archivos y en que condiciones el
                    admin puede descargarlos.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {draft && (
                    <>
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              setDraft({
                                ...draft,
                                preferredStorage:
                                  value as BackupSettingsDraft["preferredStorage"],
                              })
                            }
                          >
                            <SelectTrigger>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              setDraft({
                                ...draft,
                                cloudFolder: event.target.value,
                              })
                            }
                            placeholder="inhalex-respaldos"
                          />
                          <p className="text-xs text-muted-foreground">
                            Prefijo para ordenar respaldos completos y de
                            coleccion en el proveedor remoto.
                          </p>
                        </div>
                      </div>

                      <div className="grid gap-3">
                        <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/70 px-3 py-2.5 transition-all duration-300 hover:border-primary/30">
                          <div>
                            <p className="text-sm font-medium text-foreground">
                              Permitir descarga local
                            </p>
                            <p className="text-xs text-muted-foreground">
                              El admin solo podra bajar un respaldo a su equipo
                              si esta opcion esta activa.
                            </p>
                          </div>
                          <Switch
                        ...
                        <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/70 px-3 py-2.5 transition-all duration-300 hover:border-primary/30">
                          <div>
                            <p className="text-sm font-medium text-foreground">
                              Mantener espejo local
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Aunque el destino principal sea nube, el servidor
                              conserva una copia local para contingencias.
                            </p>
                          </div>
                          <Switch
                            checked={draft.keepLocalMirror}
                            onCheckedChange={(value) =>
                              setDraft({ ...draft, keepLocalMirror: value })
                            }
                          />
                        </div>
                      </div>
                            ...
                            <div className="space-y-1 text-sm text-muted-foreground">
                              <p>
                                ...
                                {draft.backupScope === "database"
                                  ...
                                  : draft.selectedCollections.length}
                              </p>
                              <p>
                                Documentos estimados protegidos:{" "}
                                {projectedProtectedDocuments}
                              </p>
                              <p>
                                Retencion aplicada: {draft.retentionDays} dias
                              </p>
                              <p>
                                Ultimo error registrado:{" "}
                                {settings?.lastError?.trim()
                                  ? settings.lastError
                                  : "Sin incidencias"}
                              </p>
                            </div>
                      ...
                      {draft.preferredStorage !== "local" &&
                        !isPreferredRemoteConfigured(settings) && (
                          <div className="rounded-lg border border-amber-400/40 bg-amber-500/10 p-4 text-sm text-amber-900 dark:text-amber-200">
                      ...
                      <Button
                        type="button"
                        className="w-full rounded-md py-2.5 text-sm shadow-[0_18px_45px_-22px_rgba(22,163,74,0.75)]"
                        onClick={saveSettings}
                        disabled={isSavingSettings}
                      >
                        <Save className="h-4 w-4" />
                        {isSavingSettings
                          ? "Guardando estrategia..."
                          : "Guardar estrategia"}
                      </Button>
                    </>
                  )}
            ...
            <div className="grid gap-4 xl:grid-cols-[1.1fr_1fr]">
              <Card className="transition-all duration-300 hover:shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>
                    Lanza respaldos puntuales sin esperar al siguiente ciclo
                    automatico.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-lg border border-border/60 bg-background/80 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm leading-6 text-muted-foreground">
                          Genera un paquete con todas las colecciones
                          autorizadas segun el usuario de Mongo actual.
                        </p>
                      </div>
                      <Button
                        type="button"
                    ...
                    <div className="rounded-lg border border-border/60 bg-background/80 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm leading-6 text-muted-foreground">
                          Ideal para capturas especificas de productos, usuarios
                          o cualquier modulo puntual.
                        </p>
                      </div>
              ...
              <Card className="transition-all duration-300 hover:shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>
                    Selecciona un respaldo existente, restauralo o guialo a una
                    descarga local solo si la politica lo permite.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={importMode}
                      onValueChange={(value) =>
                        setImportMode(value as AdminBackupImportMode)
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="rounded-lg border border-border/60 bg-background/80 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="space-y-2">
                        <p className="text-sm font-medium text-foreground">
                          Detalle del respaldo seleccionado
                        </p>
                        {selectedBackup ? (
                          <div className="space-y-1 text-sm text-muted-foreground">
                            ...
                            <p>
                              Destino:{" "}
                              {storageLabel(selectedBackup.storageProvider)}
                            </p>
                            <p>
                              Tamano:{" "}
                              {formatBytes(totalBackupSize(selectedBackup))}
                            </p>
                          </div>
                        ) : (
                          <p className="text-sm text-muted-foreground">
                            Aun no seleccionas un respaldo.
                          </p>
                        )}
                      </div>
                    ...
                    {selectedBackup?.remoteAvailable &&
                      ...
                      selectedBackup.remoteIdentifier && (
                        <div className="mt-3 rounded-lg border border-border/60 bg-background/70 px-3 py-2.5 text-sm text-muted-foreground">
                          <p className="font-medium text-foreground">
                            Objeto remoto privado
                          </p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Button
                      ...
                      disabled={
                        isImportingBackup || !selectedBackupId || isLoading
                      }
                    >
                  ...
                  <div className="rounded-lg border border-dashed border-border/60 bg-muted/20 p-4 text-sm text-muted-foreground">
                    <p>
                      La descarga no es obligatoria: el flujo principal es
                      generar y conservar respaldos en nube o servidor. Solo se
                      exportan localmente cuando el admin lo decide y la
                      politica lo permite.
                    </p>
                  </div>
                </CardContent>
          ...
          <TabsContent value="history" className="space-y-4">
            <div className="grid gap-4 lg:grid-cols-3">
              <Card className="border-primary/10 bg-primary/[0.03]">
                ...
                <CardContent className="text-sm text-muted-foreground">
                  Ultimos respaldos disponibles para restauracion o descarga
                  controlada.
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardTitle className="text-2xl">
                    {formatBytes(
                      backups.reduce(
                        (sum, item) => sum + totalBackupSize(item),
                        0,
                      ),
                    )}
                  </CardTitle>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardTitle className="text-base leading-6">
                    {formatDate(status?.lastFailureAt)}
                  </CardTitle>
                </CardHeader>
                <CardContent className="text-sm text-muted-foreground">
                  {status?.lastError?.trim()
                    ? status.lastError
                    : "Sin errores recientes en la automatizacion."}
                </CardContent>
              </Card>
            </div>
            ...
            <div className="space-y-4">
              {backups.length === 0 ? (
                <Card className="border-dashed border-border/70 bg-muted/10">
                  <CardContent className="flex min-h-40 flex-col items-center justify-center space-y-3 text-center">
                    <div className="rounded-full bg-primary/10 p-4 text-primary">
                      <ShieldCheck className="h-8 w-8" />
                    </div>
                    <div className="space-y-1">
                      <p className="text-lg font-semibold text-foreground">
                        Aun no hay respaldos registrados
                      </p>
                      <p className="max-w-xl text-sm text-muted-foreground">
                        Genera el primer respaldo desde la pesta�a de ejecucion
                        o activa una politica automatica para empezar a poblar
                        el historial.
                      </p>
                    </div>
                  </CardContent>
                ...
                backups.map((backup) => {
                  ...
                  const allowLocalDownload =
                    Boolean(settings?.localDownloadsEnabled) &&
                    isBackupReady(backup);
                    ...
                    >
                      <CardContent className="space-y-4 p-4">
                        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              ...
                              disabled={
                                !allowLocalDownload ||
                                Boolean(downloadingById[backup.id])
                              }
                            >
                              <Download className="h-4 w-4" />
                              {downloadingById[backup.id]
                                ? "Guardando..."
                                : "Guardar local"}
                            </Button>
                            <Button
                              ...
                              disabled={
                                !isBackupReady(backup) || isImportingBackup
                              }
                            >
                              <Upload className="h-4 w-4" />
                              Restaurar
                        ...
                        <div className="grid gap-3 lg:grid-cols-3">
                          <div className="rounded-lg border border-border/60 bg-background/80 px-3 py-2.5 text-sm">
                            <p className="font-medium text-foreground">
                              Disponibilidad
                            </p>
                            <p className="mt-1 text-muted-foreground">
                              Local: {backup.localAvailable ? "Si" : "No"} |
                              Remoto: {backup.remoteAvailable ? "Si" : "No"}
                            </p>
                          </div>
                          <div className="rounded-lg border border-border/60 bg-background/80 px-3 py-2.5 text-sm">
                            <p className="font-medium text-foreground">
