...
function collectionActivityLabel(
  value: AdminMonitoringOverview["collections"][number]["activityLevel"],
): string {
...
function userActivityLabel(
  value: AdminMonitoringOverview["users"]["spotlightUsers"][number]["activityLevel"],
): string {
...
function MetricCard({
  ...
  return (
    <Card className="admin-section-card min-h-[7rem] bg-card/90">
    ...
    <div className="grid gap-4">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div
            key={`metric-${index}`}
            className="h-24 animate-pulse rounded-lg border border-border/60 bg-card/70"
          />
        ))}
      </div>
      <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
        <div className="h-[260px] animate-pulse rounded-lg border border-border/60 bg-card/70" />
        <div className="h-[260px] animate-pulse rounded-lg border border-border/60 bg-card/70" />
      </div>
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="h-[280px] animate-pulse rounded-lg border border-border/60 bg-card/70" />
        <div className="h-[280px] animate-pulse rounded-lg border border-border/60 bg-card/70" />
      </div>
    </div>
  );
...
export function AdminMonitoringSection() {
  ...
  const [overview, setOverview] = useState<AdminMonitoringOverview | null>(
    null,
  );
    ...
    <section className="admin-panel-shell admin-animate-card">
      <div className="relative z-10">
        ...
        <Tabs value={tab} onValueChange={setTab} className="mt-6 gap-4">
          <div className="admin-toolbar-surface space-y-3 px-4 py-3 lg:sticky lg:top-0 lg:z-20">
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                ...
                <Badge variant="outline">
                  Cadencia: {REFRESH_INTERVAL_MS / 1000}s
                </Badge>
                <Badge variant="outline">
                  Ultima lectura:{" "}
                  {overview ? formatDateTime(overview.generatedAt) : "-"}
                </Badge>
              </div>
                ...
                <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
                  <Card className="admin-section-card bg-card/90">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Flujo de solicitudes agrupado cada{" "}
                        {overview.bucketMinutes} minutos.
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ChartContainer
                        ...
                        <AreaChart data={timelineData}>
                          <defs>
                            ...
                            >
                              <stop
                                offset="5%"
                                stopColor="var(--color-failedRequests)"
                                stopOpacity={0.22}
                              />
                              <stop
                                offset="95%"
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        {methodChartData.map((item) => (
                          <div
                            key={item.key}
                            className="flex items-center justify-between rounded-xl border border-border/60 bg-secondary/20 px-3 py-2 text-sm"
                          >
                    ...
                    <CardContent>
                      ...
                      >
                        ...
                        >
                          ...
                          />
                          <Bar
                            dataKey="storageSizeBytes"
                            fill="var(--color-storageSizeBytes)"
                            radius={[0, 10, 10, 0]}
                          />
                        </BarChart>
                      </ChartContainer>
                  ...
                  <Card className="admin-section-card bg-card/90">
                    ...
                    <CardContent className="space-y-5">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatBytes(overview.database.storageSizeBytes)}
                          </p>
                        </div>
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatBytes(overview.database.indexSizeBytes)}
                          </p>
                        </div>
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatDecimal(overview.runtime.processCpuPercent)}%
                          </p>
                        </div>
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatCompactNumber(
                              overview.database.uptimeSeconds / 3600,
                            )}{" "}
                            h
                          </p>
                        </div>
                      ...
                      <div className="space-y-3">
                        <div>
                          <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
                            ...
                            <span>
                              {formatBytes(
                                overview.runtime.systemMemoryBytes.used,
                              )}{" "}
                              /{" "}
                              {formatBytes(
                        ...
                        <div>
                          <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
                            ...
                            <span>
                              {formatBytes(overview.database.cacheUsedBytes)} /{" "}
                              {formatBytes(overview.database.cacheMaxBytes)}
                            </span>
                          </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <CapabilityBadge
                          enabled={overview.database.capabilities.dbStats}
                          label="dbStats"
                        />
                        <CapabilityBadge
                          enabled={overview.database.capabilities.collStats}
                          label="collStats"
                        />
                        <CapabilityBadge
                          enabled={overview.database.capabilities.serverStatus}
                          label="serverStatus"
                        />
                      </div>
                    </CardContent>
                              ...
                              <TableRow key={item.name}>
                                <TableCell>
                                  <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                      {formatDecimal(item.documentSharePercent)}
                                      % de documentos
                                    </p>
                                  </div>
                                </TableCell>
                                ...
                                <TableCell>
                                  {formatInteger(item.documents)}
                                </TableCell>
                                <TableCell>
                                  <div>
                                    <p>{formatBytes(item.storageSizeBytes)}</p>
                                    <p className="text-xs text-muted-foreground">
                                      {formatDecimal(item.sizeSharePercent)}%
                                      del total
                                    </p>
                                  </div>
                                </TableCell>
                                <TableCell>
                                  {formatBytes(item.totalIndexSizeBytes)}
                                </TableCell>
                                <TableCell>
                                  <div>
                                    <p>{formatInteger(item.totalRequests)}</p>
                                    <p className="text-xs text-muted-foreground">
                                      {formatInteger(item.failedRequests)}{" "}
                                      fallos
                                    </p>
                                  </div>
                                </TableCell>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatInteger(overview.users.total)}
                          </p>
                        </div>
                        <div className="rounded-xl border border-border/60 bg-secondary/20 p-3">
                          <p className="text-xs text-muted-foreground">
                          ...
                          <p className="mt-1 text-lg font-semibold">
                            {formatInteger(overview.users.recentlyRegistered)}
                          </p>
                        </div>
                      </div>
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <Badge variant="secondary">
                                  {formatInteger(route.totalRequests)}
                                </Badge>
                              </div>
                              <p className="mt-1 text-xs text-muted-foreground">
                                {formatDecimal(route.avgResponseTimeMs)} ms
                                promedio - {formatInteger(route.failedRequests)}{" "}
                                fallos
                              </p>
                            </div>
                          ))}
