...
function normalizeContent(content: CompanyContent): CompanyContent {
  return {
    ...
    about: {
      ...
      values: [
        ...new Set(
          content.about.values.map((value) => value.trim()).filter(Boolean),
        ),
      ],
    },
  };
...
export function AdminCompanyContentSection() {
  ...
  const [valuesInput, setValuesInput] = useState(
    DEFAULT_COMPANY_CONTENT.about.values.join("\n"),
  );
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "No se pudo cargar el contenido de la empresa";
  ...
  const hasChanges = useMemo(() => {
    return (
      ...
      JSON.stringify(normalizeContent(content))
    );
  }, [content, normalizedDraft]);

  const totalValues = useMemo(
    () => normalizeValuesInput(valuesInput).length,
    [valuesInput],
  );
  ...
  const handleLoadSuggestedTemplate = () => {
    ...
    setResultMessage(
      "Plantilla completa cargada. Guarda cambios para aplicarla en la base de datos.",
    );
    setErrorMessage("");
  };
  ...
  return (
    <div className="admin-panel-shell admin-animate-card">
      ...
      <div className="relative z-10 mt-4 grid gap-3 xl:grid-cols-[1.06fr_0.94fr]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Este bloque alimenta la pagina publica de privacidad y el
          ...
          <div className="admin-form-card">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Define las condiciones comerciales y legales visibles para el
                  cliente.
                </p>
              </div>
            </div>
          ...
          <div className="admin-form-card">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Sirve para el bloque institucional de la marca y el discurso
                  editorial del sitio.
                </p>
              </div>
            </div>
            <div className="mt-4 grid gap-3">
              ...
              <div className="grid gap-2">
                <p className="text-sm font-medium text-foreground">
                  Valores (uno por linea)
                </p>
                <Textarea
                  value={valuesInput}
                  onChange={(event) => setValuesInput(event.target.value)}
                  className="admin-input-surface min-h-[110px]"
        ...
        <div className="space-y-5 xl:sticky xl:top-24 xl:self-start">
          <div className="admin-form-card">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              Desde aqui puedes validar rapidamente si hace falta guardar,
              recargar desde base o restaurar la plantilla sugerida.
            </p>

            <div className="mt-4 grid gap-2">
              <div className="admin-stat-chip">
                <span className="font-medium">Ultimo guardado:</span>{" "}
                {formatDate(content.updatedAt)}
              </div>
              <div className="admin-stat-chip">
              ...
              <div className="admin-stat-chip">
                ...
                {canManageContent ? "Edicion completa" : "Solo lectura"}
              </div>
            </div>
          </div>
